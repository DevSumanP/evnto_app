...
allprojects {
    ...
    // kotlin-stdlib than this project's Kotlin compiler (2.3.0 in
    // settings.gradle.kts). The version mismatch makes the compiler crash on a
    // metadata check. Pin the stdlib to the compiler's version everywhere —
    // nothing here can use newer stdlib APIs anyway, so this is safe.
    configurations.all {
        resolutionStrategy {
